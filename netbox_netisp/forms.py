...
state_validator = RegexValidator(
    r'\d', message="Only input the 2 character code for your state", inverse_match=1)
...
class BillingPackageForm(BootstrapMixin, forms.ModelForm):
    class Meta:
        ...
        fields = ("name", "price", "download_speed",
                  "upload_speed", "data_cap", "slug")
...
class AntennaProfileForm(BootstrapMixin, forms.ModelForm):
    ...
    ), initial="/plugins/netbox_netisp/antenna-profile/")

    class Meta:
        model = AntennaProfile
        fields = ("name", "serial", "manufacturer",
                  "device_type", "azimuth", "beamwidth")


class RadioAccessPointForm(BootstrapMixin, forms.ModelForm):
    return_url = forms.CharField(
        widget=forms.HiddenInput(), initial="/plugins/netbox_netisp/sector/")

    class Meta:
        model = RadioAccessPoint
        fields = ("name", "manufacturer", "device_type",
                  "frequency", "antenna", "ip_address")


class CustomerPremiseEquipmentForm(BootstrapMixin, forms.ModelForm):
    return_url = forms.CharField(widget=forms.HiddenInput(
    ), initial="/plugins/netbox_netisp/customer-premise-equipment/")
...
class WirelessTicketConfirmationForm(BootstrapMixin, forms.ModelForm):
    ...
    service = forms.ModelChoiceField(
        queryset=Service.objects.all(), disabled=True)
...
class ServiceForm(BootstrapMixin, forms.ModelForm):
    ...
    billing_package = forms.ModelChoiceField(
        queryset=BillingPackage.objects.all())
    account = forms.ModelChoiceField(
        queryset=Account.objects.all(), disabled=True)
    status = forms.CharField(disabled=True)
...
class WorkOrderForm(BootstrapMixin, forms.ModelForm):
    class Meta:
        model = WirelessTicket
        fields = ("notes", "priority", "technician")
